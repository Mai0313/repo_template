...
### How `make gen-docs` Works

1. Cleans and recreates the `docs/` directory
2. Copies `README.md`, `README.zh-CN.md`, `README.zh-TW.md` into `docs/`
3. Runs `gen_docs.py` twice to generate API reference docs:
    - `./src` → `./docs/Reference` (library API)
    - `./scripts` → `./docs/Scripts` (utility scripts)

`scripts/gen_docs.py` supports `.py` and `.ipynb` files, with two modes: `class` (default, one mkdocstrings directive per class) and `file` (one per file). See `--help` for all options.
...
```bash
# If your console script is named "repo_template"
uvx repo_template
...
uv run poe main        # run CLI entry (same as uv run repo_template)
...
- Docs Deploy (`deploy.yml`)
    ...
    - Builds `zensical` site and publishes to GitHub Pages
    - Setup needed:
